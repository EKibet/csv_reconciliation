...
def reconcile_csv_util(source_path, target_path, output_directory='./media'):
    ...
    discrepancies_dfs.append(missing_in_target[[unique_identifier]].rename(columns={unique_identifier: 'Record Identifier'}).assign(Type='Missing in Target'))

    # Records missing in source
    discrepancies_dfs.append(missing_in_source[[unique_identifier]].rename(columns={unique_identifier: 'Record Identifier'}).assign(Type='Missing in Source'))
    ...
    for column in source_df.columns[1:]:  # Skip the unique identifier column
        ...
        discrepancies_dfs.append(column_discrepancies[[unique_identifier]].rename(columns={unique_identifier: 'Record Identifier'}).assign(Type='Field Discrepancy', Field=column,
    ...
    # Reorder columns
    columns_order = ['Type', 'Record Identifier', 'Field', 'Source_Value', 'Target_Value']
