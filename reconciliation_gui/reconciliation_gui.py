...
class ReconciliationApp:
    ...
    def browse_source(self):
        self.source_path = filedialog.askopenfilename(filetypes=[("CSV Files", "*.csv")])
        print(f"Source CSV: {self.source_path}")

    def browse_target(self):
        self.target_path = filedialog.askopenfilename(filetypes=[("CSV Files", "*.csv")])
    ...
    def show_result(self, output_path):
        ...
        result_label = tk.Label(result_window, text=f"Reconciliation Result\nOutput saved to: {output_path}")
        result_label.pack()

        view_button = tk.Button(result_window, text="View Result", command=lambda: self.view_result(output_path))
        view_button.pack()

    def view_result(self, output_path):
        if os.path.exists(output_path):
            os.system(f"open {output_path}")  # Opens the file with the default application
        else:
